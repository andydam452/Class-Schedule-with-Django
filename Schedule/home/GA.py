...
class GA:
            ...
            for i in self.Subject:
                for j in range((self.Subject[i])):
                    a = [i, random.choice(self.ClassRoom), random.choice(self.MeetingTime),
                         random.choice([teacher for teacher in self.Teacher if i in self.Teacher[teacher]]),
        ...
        for i in range(len(individual)):
            ...
            for j in range(len(individual)):
                ...
                if i != j and individual_sort_name[j][3] == individual_sort_name[i][3] and individual_sort_name[j][
                    ...
                    count_class_per_day = 0
                if count_class_per_day == self.Max_course_per_day:
                    score += 1
    ...
    def chonloc(self):
        ...
        self.population_score = sorted(self.population_score, key=lambda x: x[-1])
    ...
    def schedule(self):
        ...
        self.population_score = list(map(lambda x: [x, self.calcFitness(x)], population))
        self.population_score = sorted(self.population_score, key=lambda x: x[-1])
